    ...
    namespace net {
        ...
        struct SockAddr {
            ...
            inline int Port() const {
                //as address and port in socket struct are network byte order
            ...
            inline std::string AddrStr() const {
                ...
                utils::Assert(s != NULL, "connot decode address");
                return std::string(s);
            }
           ...
           */
            inline void Set(const char *host, int port) {
                ...
                //TCP Protocol, if use SOCK_DGRAM means UDP Protocol
                hints.ai_socktype = SOCK_STREAM;
                addrinfo *res = NULL;
                int sig = getaddrinfo(host, NULL, &hints, &res);
                utils::Check(sig == 0 && res != NULL, "cannot obtain address of %s", host);
                utils::Check(res->ai_family == AF_INET, "Does not support IPv6");
        ...
        public:
           ...
           */
            inline void Listen(int backlog = 32) {
           ...
           * \return size of data actually sent
           */
            inline ssize_t SendAll(const void *buf_, size_t len) {
                const char *buf = reinterpret_cast<const char *>(buf_);
                size_t ndone = 0;
                while (ndone < len) {
                    ssize_t ret = send(sockfd, buf, static_cast<sock_size_t>(len - ndone), 0);
                    if (ret == -1) {
...
//        struct SelectHelper : public boost::noncopyable {
//        public:
//            SelectHelper() {
//                FD_ZERO(&read_set);
//                FD_ZERO(&write_set);
...
//                    return select(maxfd, rfds, wfds, efds, &time);
//                }
//
//            }
//
